...
- `sync:patch` - JSON Patch sync (unified for all stores)
- `sync_role` - Assign/update user roles (DM permission management)
...
- `sync:patch_received` - **NEW** JSON Patch received from another client
- `token_added/updated/removed` - Token changes (legacy)
- `initiative_updated` - Initiative changes (legacy)
- `combat_state_changed` - Combat status (legacy)
- `map_updated` - Map changes (legacy)
- `fog_updated` - Fog changes (legacy)
