...
import type { SyncMiddlewareConfig, SyncPatchPayload, JsonPatchOperation, TransportAdapter } from './types';
...
// Middleware type definition following Zustand's pattern
type SyncPatch = <T>(
...
type SyncPatchImpl = <T>(
  config: SyncMiddlewareConfig,
  initializer: StateCreator<T, [], []>
) => StateCreator<T, [], []>;

/**
 * Implementation of the syncPatch middleware
 */
const syncPatchImpl: SyncPatchImpl = (config, initializer) => (set, get, api) => {
  const {
    channel,
    enabled = true,
    throttleMs = 0,
  ...
  const sendPatches = (patches: JsonPatchOperation[]) => {
    ...
    if (!patchTransport.isConnected()) return;

    const userId = patchTransport.getUserId();
    if (!userId) return;

    const payload: SyncPatchPayload = {
      messageId: messageIdManager.generateMessageId(userId),
      userId,
      channel,
      patches,
      timestamp: Date.now(),
  ...
  const handleRemotePatches = (payload: SyncPatchPayload) => {
    ...
    try {
      ...
      set(newState as Parameters<typeof set>[0], true);
  ...
  if (enabled) {
    patchTransport.subscribe(channel, handleRemotePatches);
  }

  // Create type-safe wrapped set function
  type SetState = typeof set;
  const syncSet: SetState = (partial, replace) => {
    ...
    if (!enabled || !patchTransport.isConnected()) {
      mwState.lastState = deepClone(nextState);
      return;
    }
 ...
 * Usage: create(syncPatch({ channel: 'tokens' })((set, get) => ({ ... })))
 */
export const syncPatch = ((config: SyncMiddlewareConfig) => 
  (initializer: StateCreator<any, [], []>) => 
    syncPatchImpl(config, initializer)
) as SyncPatch;

/**
 * Utility to manually trigger a full state sync
 * Useful for initial state broadcast when joining a session
 */
export function broadcastFullState<T extends object>(
  channel: string,
  state: T,
  transport: TransportAdapter = patchTransport
): void {
