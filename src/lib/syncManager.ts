...
class SyncManager {
  ...
  private handleSessionJoined(data: SessionJoinedPayload): void {
    ...
    if (data.users.length === 1) {
      console.log('📤 Session creator - state will sync via JSON Patch middleware');
    }
  }
  ...
  // NOTE: Type-specific sync methods removed - now handled by JSON Patch middleware (syncPatch)
  // See: src/lib/sync/syncPatchMiddleware.ts
