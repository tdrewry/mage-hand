 ...
 * Persistence handled via DurableObjectRegistry (kind: 'campaigns').
 */
    ...
    set((s) => ({
      ...
      nodePositions: {
        ...
        [campaignId]: {
          ...
          [node.id]: { x: 50 + (c(s, campaignId)?.nodes.length ?? 0) * 220, y: 100 },
        },
      },
    })),
...
// Helper used in addNode to access campaign node count
function c(s: CampaignState, campaignId: string) {
